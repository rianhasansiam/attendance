...
Only active SUPER_ADMIN users can edit or delete saved expense or balance records. `canEditDailyExpenseTransactions` and `canDeleteDailyExpenseTransactions` hide their actions for other roles; the corresponding authorization helpers enforce the same rules in the API and service. Each change rechecks and locks the persisted user role/status within the database transaction, so a stale session role cannot authorize a demoted or inactive user. ADMIN users retain access to viewing, adding records, and managing categories.
